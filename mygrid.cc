...
*/
void MyGridAgent::find_nearest_sink(const char* packet_type, float xpos, float ypos, float x_event, float y_event, int id_sink)
{
...
/*
void MyGridAgent::movement()
{	float new_x=100;
	float new_y=100;
	int nearsink_id=2;
	char out[100];
	Tcl& tcl = Tcl::instance(); 
	sprintf(out, "%s movement3 %d %f %f %f", name(), nearsink_id, CURRENT_TIME, new_x, new_y); 
	printf("current time is %f", CURRENT_TIME);
	tcl.eval(out);
}
*/ 
...
//                        float energy = iEnergy;
        ...
        else if (argc == 8) {                            //send data message
		if (strcmp(argv[1], "send") == 0) {
                        float x_s = atof(argv[3]);
                        float y_s = atof(argv[4]);
                        float x_event = atof(argv[5]);
                        float y_event = atof(argv[6]);
                        int id_sink = atoi(argv[7]);
                        find_nearest_sink(argv[2],x_s,y_s,x_event,y_event,id_sink);
